...
try:
    from backend.services.telegram_service import notify_affair_unlinked as _tg_unlinked
    _tg_notifs_ok = True
except ImportError:
    try:
...
async def verify_linked_articles(affair_id: str, auto_unlink: bool = Query(False), user: Dict = Depends(_require_admin)):
    ...
    try:
        from backend.services.ai_groq_service import verify_linked_articles as _verify_articles
    except ImportError:
        try:
