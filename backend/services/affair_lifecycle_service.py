...
try:
    from backend.services.push_service import notify_new_affair as _push_notify
    _push_ok = True
except ImportError:
    try:
...
# Dédup IA (optionnel)
try:
    from backend.services.ai_groq_service import detect_duplicate_affairs as _ai_dedup
    from backend.services.ai_groq_service import validate_article_affair_relevance as _ai_relevance
    from backend.services.ai_groq_service import detect_stale_active_matches as _ai_stale_active
    from backend.services.ai_groq_service import match_article_to_affairs as _ai_match_article
    from backend.services.ai_groq_service import detect_commune_ai as _ai_detect_commune
    from backend.services.ai_groq_service import rewrite_affair_title as _ai_rewrite_title
    _ai_dedup_ok = True
...
try:
    from backend.services.entity_aliases import correct_text_stt as _correct_stt, correct_entities_list as _correct_entities, resolve_entity as _resolve_entity
...
class AffairLifecycleService:
    ...
    def _create_clusters_with_ai(self, candidates: List[Dict]) -> List[Dict]:
        ...
        """
        try:
            from backend.services.ai_groq_service import cluster_articles_with_ai, is_available
            if not is_available():
                return []
        except ImportError:
            try:
    ...
    def run_simple_cycle(self) -> Dict[str, Any]:
        ...
        _ai_available = False
        try:
            from backend.services.ai_groq_service import is_available as _ai_is_available
    ...
    def generate_affair_context(self, affair_id: str) -> Dict:
        ...
        from bson import ObjectId
        try:
            from backend.services.ai_groq_service import generate_affair_context as _gen_ctx
        except ImportError:
            try:
    ...
    def _enrich_radio_transcriptions(self) -> int:
        ...
        try:
            from backend.services.ai_groq_service import split_radio_transcription
        except ImportError:
            try:
    ...
    def run_ai_managed_cycle(self) -> Dict[str, Any]:
        ...
        # 4. Appel IA
        try:
            from backend.services.ai_groq_service import manage_affairs_with_ai
        except ImportError:
            try:
    ...
    def _pairwise_similarity(
        ...
        if entities_a or entities_b:
            # Résolution d'alias
            try:
                from backend.services.entity_aliases import entities_match
